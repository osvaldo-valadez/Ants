...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while moving_place != None and moving_place != hive :
            if m >= self.min_range and m <= self.max_range:
                ant = random_or_none(moving_place.bees)
                if ant:
                    return ant
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        copy_of_my_bees = list(self.place.bees)
        for bee in copy_of_my_bees:
            bee.reduce_armor(self.damage)
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while exit != None:
                if exit.ant and exit.ant not in QueenAnt.doubled:
                    exit.ant.damage *= 2
                    QueenAnt.doubled.append(exit.ant)
                if exit.ant and exit.ant.is_container and exit.ant.contained_ant and exit.ant.contained_ant not in QueenAnt.doubled:
                    exit.ant.contained_ant.damage *= 2
                    QueenAnt.doubled.append(exit.ant.contained_ant)
                exit = exit.exit
